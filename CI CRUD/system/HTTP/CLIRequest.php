	...
	protected function parseCommand()
	{
		...
		foreach ($args as $i => $arg)
		{
			if (mb_strpos($arg, '-') !== 0)
			{
				...
				{
					$this->segments[] = filter_var($arg, FILTER_SANITIZE_SPECIAL_CHARS);
				}

				continue;
			}

			$arg   = filter_var(ltrim($arg, '-'), FILTER_SANITIZE_SPECIAL_CHARS);
			$value = null;

			if (isset($args[$i + 1]) && mb_strpos($args[$i + 1], '-') !== 0)
			{
				$value       = filter_var($args[$i + 1], FILTER_SANITIZE_SPECIAL_CHARS);
				$optionValue = true;
			}
